#!/usr/bin/env python3
"""Hierarchical NB diagnostics for Manhattan zones (weekday/weekend × rush/off-peak)."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--hier-eb",
        action="store_true",
        help="Enable Empirical Bayes shrinkage of NB r parameter across zones.",
    )
    parser.add_argument(
        "--aggregate-manhattan",
        action="store_true",
...
def fit_gamma_prior(r_values: List[float]) -> Tuple[float, float]:
    finite = [r for r in r_values if np.isfinite(r) and r > 0]
    if len(finite) < 2:
        return float("nan"), float("nan")
    mean = np.mean(finite)
    var = np.var(finite, ddof=0)
    if var <= 0:
        return float("nan"), float("nan")
    alpha = mean ** 2 / var
    beta = mean / var
...
def shrink_r(r_raw: float, alpha: float, beta: float) -> float:
    if not (np.isfinite(r_raw) and np.isfinite(alpha) and np.isfinite(beta)):
        return r_raw
    # Posterior mean for Gamma(alpha, beta): (alpha + r_raw) / (beta + 1)
    return (alpha + r_raw) / (beta + 1.0)
